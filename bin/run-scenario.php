...
// Resolve prompt: explicit text, legacy one-shot Improve, or production two-step.
$two_step = false;
$prompt = '';
$prompt_version = '';
if (($scenario['prompt_source'] ?? '') === 'ImprovePageTwoStep') {
	...
	$prompt_version = 'scenario:' . $canonical_id;
} else {
...
$turn_id = sprintf('scenario-%s-%d-%d', preg_replace('/[^a-z0-9-]+/i', '-', $canonical_id), $post_id, time());
...
$summary_path = sprintf('%s/awpt-scenario-%s-post-%d.json', $out_dir, $slug, $post_id);
$raw_path = sprintf('%s/awpt-scenario-%s-post-%d.raw.json', $out_dir, $slug, $post_id);
...
if ($two_step) {
	...
	$eval_result = $runtime->handle_message($session_id, AWPT\Support\ImprovePagePrompt::evaluate_text(), [
	...
	if (is_wp_error($eval_result)) {
		...
		$plan_text = trim((string) ($eval_result['content'] ?? ''));
		...
		echo 'plan_chars=' . strlen($plan_text) . " eval_actions=" . count($eval_actions) . " eval_propose={$eval_propose}\n";
		echo "PHASE act turn={$act_turn_id}\n";
		$result = $runtime->handle_message(
			$session_id,
			AWPT\Support\ImprovePagePrompt::act_message($plan_text),
			[
				'turn_id' => $act_turn_id,
				'focus_post_id' => $post_id,
			],
...
$run_meta = [
	...
	'soft_expected_tools' => is_array($scenario['soft_expected_tools'] ?? null) ? $scenario['soft_expected_tools'] : [],
...
$path_match = null === $soft_path || '' === $soft_path
	? null
	: ((string) $classification['path_used'] === (string) $soft_path);

$post_after = get_post($post_id);
$content_hash_after = $post_after ? hash('sha256', (string) $post_after->post_content) : null;
$sections_after = $post_after
	? AWPT\Support\PageSectionModel::from_content((string) $post_after->post_content)
	: [];
...
echo 'scorecard_prepare=' . (int) ($summary['scorecard']['prepare_change_success'] ?? 0)
	. ' replace=' . (int) ($summary['scorecard']['propose_replace_success'] ?? 0)
	. ' freehand=' . (!empty($summary['scorecard']['freehand_provenance']) ? 'yes' : 'no')
	. "\n";
if ($two_step) {
