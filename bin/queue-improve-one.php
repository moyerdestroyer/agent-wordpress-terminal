...
if ($post_id <= 0) {
	fwrite(STDERR, "Usage: wp eval-file bin/queue-improve-one.php <post_id> [one-shot] [reuse-session]\n");
	exit(1);
}
...
$base_turn = sprintf('queue-%d-%d', $post_id, time());
$out_dir = WP_CONTENT_DIR . '/plugins/agent-wordpress-terminal/tmp-queue-runs';
if (!is_dir($out_dir) && !wp_mkdir_p($out_dir)) {
	$out_dir = '/tmp';
}
$summary_path = sprintf('%s/awpt-queue-%d.json', $out_dir, $post_id);
$raw_path = sprintf('%s/awpt-queue-%d.raw.json', $out_dir, $post_id);
...
if (!$one_shot) {
	...
	$act_prompt = AWPT\Support\ImprovePagePrompt::act_message($plan_text);
	$result = $runtime->handle_message($session_id, $act_prompt, [
		'turn_id' => $act_turn_id,
		'focus_post_id' => $post_id,
	]);
	$turn_id = $act_turn_id;
...
$top_level_section_count = count(
	AWPT\Support\PageSectionModel::from_content((string) $post->post_content, [
