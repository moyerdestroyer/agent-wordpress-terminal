...
final class ToolCatalogFormatter {
    ...
    private const ABILITY_DESCRIPTIONS = [
        ...
        'awpt/propose-new-post' => 'Stages or revises a draft post/page for approval. Pass body-only post_content (no duplicate post_title heading). Prefer pattern_mode adapted (default when pattern_name is set): read the pattern, put one complete customized composition in post_content, and use pattern_name as provenance only — AWPT will not inject the raw pattern. pattern_mode prepend is only for an unchanged pattern plus an optional short body-only tail; a filled second layout under prepend is rejected. AWPT automatically targets the newest compatible open proposal for corrective requests. A pasted image requested in the design must appear in an inline Image/Cover block; featured_image_id alone is insufficient. AWPT repairs unambiguous Gutenberg wrapper and image-class serialization issues before staging and returns repairs_applied; creative copy and layout remain yours.',
