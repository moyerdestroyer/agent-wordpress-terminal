...
final class ToolCatalogFormatter {
    ...
    private const ABILITY_DESCRIPTIONS = [
        ...
        'awpt/read-theme-file' => 'Reads a relative path under the active/parent theme. Prefer paths from list/search-knowledge (theme: labels).',
        'awpt/inspect-frontend' => 'Fetches a same-site page: stylesheets, class inventory, layout signals, HTML snippet, recommended next tools.',
        ...
        'awpt/list-patterns' => 'Lists compatible registered and reusable patterns with active-theme ownership; theme-native patterns rank first among equally relevant matches.',
        'awpt/read-pattern' => 'Reads a pattern composition, role, hash, and theme dependencies before reuse. Pass purpose for supplemental reads after selecting a primary layout.',
        ...
        'awpt/search-knowledge' => 'Returns session-novel Knowledge chunks plus compact known matches. Pass purpose for refined searches; stop when exhausted is true.',
        'awpt/list-knowledge-sources' => 'Summarizes the Knowledge index (counts, stale flag, sample theme: labels) so you can discover what is indexed before searching.',
        'awpt/read-knowledge' => 'Reads a specific Core Knowledge or legacy guideline record by WordPress post ID.',
        'awpt/propose-content-update' => 'Stages a proposed post update for approval. Preserve existing structure for ordinary edits; for a substantial layout rewrite, prefer a read theme/reusable pattern and pass its name as provenance, or explain a Core/custom fallback.',
        ...
        'awpt/propose-pattern-insert' => 'Stages insertion of an unchanged registered/reusable pattern into an existing post for admin approval. After insert, replace placeholder copy in place with content-update/block tools; do not append a second filled clone of the same section.',
        ...
        'awpt/propose-new-post' => 'Stages or revises a draft post/page for approval. Pass body-only post_content (no duplicate post_title heading). Prefer active/parent-theme or reusable patterns and pattern_mode adapted (default when pattern_name is set): read the pattern, put one complete customized composition in post_content, and use pattern_name as provenance only. Core/plugin/custom composition remains allowed; when preferred patterns exist, pass pattern_fallback_reason. pattern_mode prepend is only for an unchanged pattern plus an optional short body-only tail. AWPT automatically targets the newest compatible open proposal for corrective requests. A pasted image requested in the design must appear in an inline Image/Cover block; featured_image_id alone is insufficient.',
        ...
        'awpt/propose-custom-css-update' => 'Stages Additional CSS (Customizer custom_css) for the active theme for admin approval. Prefer for small frontend layout fixes.',
    ...
    public function get_system_prompt_catalog(): string {
        ...
        $lines = [
            ...
            'Discover before answering site-specific questions: use awpt/list-knowledge-sources and/or awpt/search-knowledge for durable knowledge, theme docs, and indexed files; do not invent theme paths or pattern slugs.',
            ...
            'Pattern edits on an existing post: use awpt/list-patterns, then awpt/read-pattern before staging awpt/propose-pattern-insert. Replace placeholder copy in place afterward; do not append a duplicate filled section. For new pages, prefer propose-new-post with pattern_mode adapted and a single customized composition.',
